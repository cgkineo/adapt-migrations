...
export default class Task {
  ...
  async run ({ fromPlugins, originalFromPlugins, toPlugins, journal }) {
  ...
  async isApplicable ({ fromPlugins, originalFromPlugins, toPlugins, journal }) {
    Task.isRunning = true
    Task.stackUp(this)
    this.context = new TaskContext({
      fromPlugins,
  ...
  static async runApplicable ({ cwd = process.cwd(), fromPlugins, originalFromPlugins, toPlugins, journal }) {
    ...
    while (true) {
      ...
      for (const task of clonedTasks.filter(item => !item.isComplete)) {
        const isApplicable = await task.isApplicable({ cwd, fromPlugins, originalFromPlugins, toPlugins, journal })
      ...
      for (const task of toRun) {
        const { hasErrored } = await task.run({ cwd, fromPlugins, originalFromPlugins, toPlugins, journal })
        if (hasErrored) {
          journal.undoToIndex(lastJournalEntryIndex)
          return
        }
