...
export default class Task {
  ...
  async run ({ journal }) {
    ...
    this.context = new TaskContext({
      fromPlugins: journal.fromPlugins,
      originalFromPlugins: journal.originalFromPlugins,
      toPlugins: journal.toPlugins,
  ...
  static async runApplicable ({ cwd = process.cwd(), journal }) {
    ...
    while (true) {
      ...
      for (const task of clonedTasks.filter(item => !item.isComplete)) {
        const isApplicable = await task.isApplicable({ cwd, journal })
      ...
      for (const task of toRun) {
        const { hasErrored } = await task.run({ cwd, journal })
        if (hasErrored) {
          journal.undoToIndex(lastJournalEntryIndex)
          return
        }
  ...
  static async runTests ({ cwd = process.cwd(), journal }) {
